...
for i in {1..60}; do
    if curl -s http://$EUREKA_HOST:$EUREKA_PORT/actuator/health | grep -q "UP"; then
        echo "Eureka Server is available!"
        break
    fi
    echo "Waiting for Eureka Server... attempt $i"
    sleep 10
done
...
mkdir -p /tmp/netflix-oss/services/$SERVICE_NAME/src/main/java/com/netflix/oss/backend/controller
...
cat > /tmp/netflix-oss/services/$SERVICE_NAME/src/main/java/com/netflix/oss/backend/BackendApplication.java << 'JAVAEOF'
package com.netflix.oss.backend;
...
# Create controller
cat > /tmp/netflix-oss/services/$SERVICE_NAME/src/main/java/com/netflix/oss/backend/controller/BackendController.java << 'JAVAEOF'
...
[Unit]
Description=Netflix OSS Backend Service
